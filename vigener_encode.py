...
template = {"comment": "{comment}", "text": """[html]<p>Зашифруйте сообщение</p>
<pre>{line}</pre>
<p>шифром Виженера с&nbsp;ключом</p>
<pre>{key}</pre>""", "answer": "{answers}", "name": "{name}"}

result = list()

for line, key in en+ru:
    answer = shiffr.vigener(line, key, "encode")
    answers = list()
